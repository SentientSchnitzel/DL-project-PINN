...
def pde_loss(model: nn.Module, batch_features, device, v, epoch):
    """
    ...
    u_tt - v**2 * u_xx = 0
    """
    cutoff = int(batch_features.shape[0] * epoch / epochs)
    batch_features_cut = batch_features[:cutoff, :]

    prediction = model(batch_features_cut)
...
def train(pinn, criterion, optimizer, dataloader, boundaries, v, epochs, exp_folder, adaptive, device, scheduler):
    """
    pinn:       PINN model
    ...
    for epoch in tqdm(range(epochs), desc='Training progress', ):
        ...
        for batch_features in dataloader:
            ...
            # PDE physics loss
            batch_loss_pde = pde_loss(pinn, batch_features, device, v=v, epoch=epoch)
...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(pinn.parameters(), lr=1e-3, weight_decay=1e-4)

    enable_scheduler = True
    # scheduler = ReduceLROnPlateau(optimizer, patience=100, factor=0.5, verbose=True)
    # scheduler = StepLR(optimizer, step_size=100, gamma=0.5)
    scheduler = LinearLR(optimizer=optimizer, start_factor=1, end_factor=0.1, total_iters=500, last_epoch=-1, verbose=False) if enable_scheduler else None
    ...
    if enable_scheduler:
        ...
        Start factor: {scheduler.start_factor}
        End factor: {scheduler.end_factor}
        Total iterations: {scheduler.total_iters}
        last epoch: {scheduler.last_epoch}
        """
    else:
        settings += f'No scheduler used.'

    settings += f"""
    ### TRAINING ###
    Training with parameters \n {pinn}
    optimizer: {optimizer}
    adaptive weights: {adaptive_weights}

    number of epochs: {epochs}
    Mini batching: {mini_batching}
    batch_size: {batch_size}

    ### DATA GENERATION ###
    Boundary values of x: {boundaries}
    Time duration (T): {T}
    Number of samples: {num_samples}
    Wave speed (v): {v}

    Cutoff: {enable_cutoff}
    """

    with open (os.path.join(exp_folder, 'experiment_settings.txt'), 'w') as f:
        f.write(settings)
    ...
    train(pinn=pinn, 
          ...
          adaptive=adaptive_weights, device=device)

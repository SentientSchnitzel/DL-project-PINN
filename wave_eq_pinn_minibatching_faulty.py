...
def pde_loss(model: nn.Module, batch_features, device, v, epoch, epochs):
    """
    ...
    # batch_features = batch_features[:cutoff, :]
...
if __name__ == '__main__':
    ...
    v = 1                   # wave speed
    batch_size = 128 if mini_batching else num_samples
    ...
    # sorted_features_ = features_[torch.argsort(features_[:, 1])].clone()
    ...
    optimizer = optim.Adam(pinn.parameters(), lr=1e-2, weight_decay=1e-3)
    ...
    # TensorBoard writer
    if tensorboard:
        writer = SummaryWriter(os.path.join('runs', exp_id))
